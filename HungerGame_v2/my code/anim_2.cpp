...
void updateChicken() {
    ...
    if(TIME == 0) {
        ...
        {chickenTimeCounter = 33;}
        // camera is looking at wolf
        model[C] = tmodel[C] * RotateY(180 * (TIME)) * Translate(0, 0, -5) * RotateY(90);
        
    }
    else if(TIME < 33) {
        
        if(chickenTimeCounter == 33)
        {
            chickenTimeCounter = 40;
            Positions[C] = model[C][3];
            std::cout << Positions[C] << std::endl;
        }
        vec4 pc = Positions[C];
        vec4 dest = Positions[W];
        vec4 v = dest - pc;
        ...
        if(TIME < 28) {
            parts[C_WING_L] = tparts[C_WING_L] * RotateZ(-135 * scale(C, 27, 1));
            parts[C_WING_R] = tparts[C_WING_R] * RotateZ(135 * scale(C, 27, 1));
            
        }
        else if(TIME < 30) {
            parts[C_WING_L] = tparts[C_WING_L] * RotateZ(-135 + 135 * scale(C, 28, 2));
            parts[C_WING_R] = tparts[C_WING_R] * RotateZ(135 - 135 * scale(C, 28, 2));
            model[C] = tmodel[C] * Translate(0, 4 * sin(180 * DegreesToRadians * scale(C, 28, 2)), 0);
            model[C] *= RotateX(-90 * scale(C, 28, 2));
        }
        else if(TIME < 33) {
            model[C] = tmodel[C] * RotateX(-90);
			colors.pop();
            set_color(1,0.75 - 0.75 * sin(180 * DegreesToRadians * scale(C,30,3)),0);
		}
