...
void    updateCamera();
void shotStick(double start, double t , vec4 dest);
void moveforward(double start, double t, vec4 dest) ;
void moveforward_leg_r(double start, double t);
void moveforward_leg_l(double start, double t);
void rotate_rarm(double start, double t);
void rotate_larm(double start, double t);
void raise_larm(double start, double t, double degree);
void rotate_head(double start, double t, vec4 position);
/*        _myvariables      */
...
void rotate_head(double start, double t, vec4 position) {
	// not yet or already passed
	if(TIME < start || TIME > start + t) {
		return;
	}
	double degree = calcDegree(Positions[H], position);
	parts[H_HEAD] = tparts[H_HEAD] * RotateY(degree * scale(H,start,t));	
}
...
void raise_larm(double start, double t, double degree) {
	...
	parts[H_ARM_L] = tparts[H_ARM_L] * RotateX(-1 * degree * scale(H, start, t));
}
...
void rotate_larm(double start, double t) {
	...
	parts[H_ARM_L] = tparts[H_ARM_L] * RotateX(30 * sin(DegreesToRadians * HUNTER_SPEED * (TIME-start)) );
}
...
void rotate_rarm(double start, double t) {
	...
	parts[H_ARM_R] = tparts[H_ARM_R] * RotateX(-30 * sin(DegreesToRadians * HUNTER_SPEED * (TIME-start)) );
}
...
void moveforward(double start, double t, vec4 dest) {
	...
	if(!isMoving) {
		tPositions[H] = getPosition(model[H]);
		isMoving=true;
		mfdegree = calcDegree(tPositions[H], dest);
		mfdistance = calcDistance(tPositions[H], dest);
	}

	rotate_larm(start, t);
	rotate_rarm(start, t);
	moveforward_leg_l(start, t);
	moveforward_leg_r(start, t);
	model[H] = Translate(tPositions[H]) * RotateY(mfdegree) * 
		Translate(0,0,mfdistance * scale(H, start,t));
...
void shotStick(double start, double t , vec4 dest){
	...
	if(!isShoting) {
		tPositions[H] = getPosition(model[H]);
		ssdegree = calcDegree(tPositions[H], dest);
		ssdistance = calcDistance(tPositions[H], dest);
		isShoting = true;
	}
	raise_larm(start, t*3/4, 210);
	raise_larm(start + t*3/4, t/4, -210);	
}
